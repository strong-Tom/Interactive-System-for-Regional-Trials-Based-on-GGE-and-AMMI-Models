...
class TreeUtil():
    ...
    def printDetail(self, return_data):
        ...
        for match_node_id in match_node_leaves:
            ...
            guize.append(list)
            # print(lines)
        # df = pd.DataFrame(guize)
        # names = ['node_id','num','num_pct','class','rule']
        # df = pd.DataFrame(columns=names, data=guize)
        df = pd.DataFrame(data=guize)
...
class saveRule(APIView):

    def save_file(self, file_name):
        return self.request.build_absolute_uri(settings.MEDIA_URL + 'rule_table/' + file_name)

    def post(self,request):
        #前端传过来的多选框勾选到的第一个文件的信息
        # file=json.loads(request.body)[0]
        with open('./zyq/utils/rule_json.json', 'r', encoding='utf-8') as fw:
            injson = json.load(fw)
        ...
        for file_name in file_list:
            ...
            file_url = self.save_file(file_name)
            name = models.rule_table.objects.filter(name=file_name)
            if not name:
                # 存到数据库中的路径，相对路径即可，存绝对路径会出错
                models.rule_table.objects.create(
                    file='rule_table/' + file_name,
                    name=file_name
                )
        return Response({"rule_json": injson})
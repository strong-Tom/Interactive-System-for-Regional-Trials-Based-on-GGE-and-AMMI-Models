# Data Description
The `clone_information_of_poplar_varieties.xlsx` file contains clone information for poplar trees, covering 1548 samples of tree height and diameter across three locations. `scatter_data.xlsx` and `clone_information_of_poplar_varieties.xlsx` are essentially the same file, used to visualize poplar clone information through scatter plots to clearly display the growth conditions of specific varieties at specific locations. The `Environmental_factors.xlsx` file includes environmental factor data for these three locations. The `gge_ammi_data.csv` dataset is used for analysis with GGE and AMMI models, derived from processing the `clone_information_of_poplar_varieties.xlsx` dataset. It includes 43 rows and 3 columns, where each row represents a variety and each column represents a location, with values representing tree height. We use the average tree height of each variety at each location as a measure of yield, opting not to use diameter information as changes in tree height are generally more significant than changes in diameter.
